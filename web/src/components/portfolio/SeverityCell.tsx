...
// Rectangular HUD count chips mirroring the severity tones, with a small colored
// dot. Square corners and mono tabular figures match the "CLASSIFIED" terminal.
const chip =
  "inline-flex items-center gap-1.5 rounded-sm border px-2 py-0.5 font-mono text-xs font-medium tabular-nums";

const tones: Record<Severity, { chip: string; dot: string }> = {
  critical: { chip: "border-critical/30 bg-critical/15 text-critical", dot: "bg-critical" },
  major: { chip: "border-major/30 bg-major/15 text-major", dot: "bg-major" },
  minor: { chip: "border-minor/30 bg-minor/15 text-minor", dot: "bg-minor" },
};
