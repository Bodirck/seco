...
Public data only, reproducible from scratch. The three public sources (EUBUCCO Luxembourg building footprints, STATEC building permits, and the ACT commune boundaries used to resolve each building's real commune) are described in [docs/data-sources.md](docs/data-sources.md).

The buildings are real: footprints and coordinates from EUBUCCO Luxembourg, the footprint area computed from the real outline, the commune resolved from those coordinates, plus EUBUCCO's building use (mostly model-estimated, partly from OpenStreetMap) and floor count (model-estimated). Height is real for most buildings (Luxembourg cadastre, about 78%) and model-estimated for the rest. What stays synthetic is the identity EUBUCCO does not provide (building name, street address) and, above all, the inspection reports themselves: the defects are invented from a catalogue, so a building's location is real but its condition is fictional. The inspection reports are generated by us with reportlab, because no public corpus of real inspection reports exists at volume. They still follow a real industry taxonomy (RICS C1/C2/C3 condition ratings) and real disciplines (structure, roofing, facade, fire safety, HVAC, electrical). This is a deliberate, documented choice: in production the generator is swapped for real SECO reports without changing the rest of the pipeline. When a building is added from the public registry, its commune is the real one, resolved from the coordinates by point-in-polygon against the official ACT commune boundaries (data.public.lu, CC0); only the building name and street address stay synthetic, since EUBUCCO has none for Luxembourg. Extracting defects always requires an inspection PDF.
...
- a per-building view in three tabs (case file with a locator map placing the building in Luxembourg from its public coordinates, KPI charts, and a defect log you can sort and filter per column), a command bar to ask a question about that building from any tab, explanatory tooltips on every indicator, and Excel or PDF report export. The case file surfaces the real EUBUCCO attributes (use, floors, footprint area, height), each labelled as measured or model-estimated;
...
- an Import page to add a building three ways (attach to an existing building, pull one from the EUBUCCO public registry by searching its commune, or create one) and upload its inspection PDF to extract, score and index on the spot.
...
Données publiques uniquement, reproductible depuis zéro. Les trois sources publiques (empreintes de bâtiments EUBUCCO Luxembourg, permis de bâtir STATEC, et les limites communales ACT utilisées pour déduire la vraie commune de chaque bâtiment) sont décrites dans [docs/data-sources.md](docs/data-sources.md).

Les bâtiments sont réels : empreintes et coordonnées issues d'EUBUCCO Luxembourg, la surface au sol calculée depuis l'empreinte réelle, la commune déduite de ces coordonnées, plus l'usage du bâtiment (surtout estimé par modèle, en partie issu d'OpenStreetMap) et le nombre d'étages d'EUBUCCO (estimé par modèle). La hauteur est réelle pour la plupart des bâtiments (cadastre luxembourgeois, ~78%) et estimée par modèle pour le reste. Ce qui reste synthétique, c'est l'identité qu'EUBUCCO ne fournit pas (nom du bâtiment, adresse de rue) et, surtout, les rapports d'inspection eux-mêmes : les défauts sont inventés à partir d'un catalogue, donc la localisation d'un bâtiment est réelle mais son état est fictif. Les rapports d'inspection sont fabriqués par nous avec reportlab, car il n'existe pas de corpus public de vrais rapports d'inspection en volume. Ils suivent malgré tout une vraie taxonomie métier (notes de condition RICS C1/C2/C3) et de vraies disciplines (structure, toiture, façade, sécurité incendie, CVC, électricité). C'est un choix assumé et documenté : en production, le générateur est remplacé par les vrais rapports SECO sans toucher au reste du pipeline. Pour un bâtiment ajouté depuis le registre public, la commune est la vraie, déduite des coordonnées par point-in-polygon contre les limites communales officielles de l'ACT (data.public.lu, CC0) ; seuls le nom et l'adresse de rue restent synthétiques, EUBUCCO n'en fournissant pas pour le Luxembourg. L'extraction des défauts requiert toujours un PDF d'inspection.
...
- une vue par bâtiment en trois onglets (dossier avec une carte qui situe le bâtiment au Luxembourg à partir de ses coordonnées publiques, graphes KPI, et un journal des défauts triable et filtrable par colonne), une barre de commande pour interroger ce bâtiment depuis n'importe quel onglet, des infobulles sur chaque indicateur, et un export de rapport Excel ou PDF. Le dossier affiche les vrais attributs EUBUCCO (usage, étages, surface au sol, hauteur), chacun étiqueté mesuré ou estimé par modèle ;
...
- une page Import pour ajouter un bâtiment de trois façons (rattacher à un bâtiment existant, en tirer un du registre public EUBUCCO en cherchant sa commune, ou en créer un) et charger son PDF d'inspection pour l'extraire, le scorer et l'indexer immédiatement.
