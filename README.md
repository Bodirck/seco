...
The buildings are real: footprints, heights, and coordinates from EUBUCCO Luxembourg. The inspection reports are generated by us with reportlab, because no public corpus of real inspection reports exists at volume. They still follow a real industry taxonomy (RICS C1/C2/C3 condition ratings) and real disciplines (structure, roofing, facade, fire safety, HVAC, electrical). This is a deliberate, documented choice: in production the generator is swapped for real SECO reports without changing the rest of the pipeline. When a building is added from the public registry, its commune is the real one, resolved from the coordinates by point-in-polygon against the official ACT commune boundaries (data.public.lu, CC0); only the building name and street address stay synthetic, since EUBUCCO has none for Luxembourg. Extracting defects always requires an inspection PDF.
